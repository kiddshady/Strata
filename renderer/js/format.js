...
/**
 * Un entero EXACTO, agrupado por miles: 1.234.567. Es el conteo que no se
 * puede abreviar — "filas: 1,2M" no le sirve a quien busca la fila 1.234.001.
 * Acá la agrupación es el punto: en es-AR el separador de miles es el punto y
 * no hay decimales con los que confundirlo. (Strata)
 */
export function fmtInt(n) {
  if (n === null || n === undefined || !Number.isFinite(Number(n))) return '—';
  const clave = `${locale.tag}|int`;
  let f = formateadores.get(clave);
  if (!f) {
    f = new Intl.NumberFormat(locale.tag, { maximumFractionDigits: 0, useGrouping: true });
    formateadores.set(clave, f);
  }
