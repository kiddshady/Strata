...
/* ═══════════════════════════════════════════════════════════════════════════
   OPAL — almacenamiento (en Strata: ajustes y recientes)
...
const ROOT = process.env.STRATA_DATA || defaultRoot();
const SETTINGS_FILE = path.join(ROOT, 'settings.json');

/* ── Ajustes de Strata ───────────────────────────────────────────────────────
   El único lugar donde se declara qué se guarda. Lo que se agregue acá aparece
   solo en instalaciones viejas gracias a withDefaults. */
const SCHEMA = 2;

const DEFAULT_SETTINGS = {
  schema: SCHEMA,
  /** Alto de fila del grid: 'comoda' (28px) o 'compacta' (24px). */
  densidad: 'comoda',
  /** Al arrancar, reabrir la base que quedó abierta al cerrar. */
  reabrir: true,
  /** La base abierta al cerrar la app; null si se cerró a mano. */
  ultimaBase: null,
  /** El panel de fila a la derecha del grid. */
  inspector: true,
  /** Lo último que se escribió en la consola SQL. */
  consulta: '',
};
...
const MIGRATIONS = {
  // La 1 era la plantilla de Opal (autoGuardado, ultimaVista…): de ahí solo
  // sirve la densidad, y solo si es un valor que Strata entiende.
  1: (data) => ({
    schema: 2,
    ...(['comoda', 'compacta'].includes(data.densidad) ? { densidad: data.densidad } : {}),
  }),
};
